...
- T-MCFG1: every `STRONG_TIER_STAGES` member (`extract`, `canonicalize`, `graph`) falls back to `DISTIL_MODEL` when set, and raises `RuntimeError` when it isn't — the same "must be set explicitly" contract `AnthropicClient` already enforces, just resolved per stage.
