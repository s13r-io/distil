...
- T-Y17 (external-collector queue): `is_bot_check_refusal` is a substring match on yt-dlp's verbatim `"Sign in to confirm you're not a bot"` line (works against a `YoutubeFetchError` or a plain string) — true for that exact challenge text, false for a no-captions error and false for a playlist-listing failure. See `AGENTS.md`'s collector-queue entry for how confident this discrimination is (high in practice, not structural — a reword just stops the match, never mis-parks an unrelated failure).
