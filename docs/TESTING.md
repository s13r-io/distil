...
- T-Y10 (Phase 20): with `DISTIL_YOUTUBE_API_KEY` set (via `monkeypatch`), both `list_playlist_video_urls` and `_fetch_into` pass `--extractor-args` containing `innertube_key=<value>` and `innertube_host=youtubei.googleapis.com` alongside `player_client=android_vr,web_safari`; with the env var unset, the command line is unchanged from T-Y8.
