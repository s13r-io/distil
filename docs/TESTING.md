...
### triage.py (unit, FakeClient — standalone classifier, not called by the pipeline anymore)
...
- T-E10 (owner decision — triage+extract merged into one call, `run_triage_extract`): a single `FakeClient` response containing a `<TRIAGE>...</TRIAGE><ITEMS>...</ITEMS>` two-section reply parses into both a `Triage` and a list of `KnowledgeItem`s from exactly one `client.complete` call — the strong model reads the transcript once, not twice.
- T-E11: a response whose `<ITEMS>` array is truncated mid-stream (missing `</ITEMS>`, cut off by the output-token cap) still recovers whatever complete leading items parsed via the same `_parse_items_json`/`_recover_truncated_leading_objects` path `run_extraction` already used — reuse, not reimplementation, of T-E5's guarantee.
- T-E12: a response missing a `<TRIAGE>` section entirely, or missing `<ITEMS>` entirely, raises a `ParseError` that `_complete_triage_extract_with_retry` retries exactly like a dropped connection (mirrors T-E7); exhausting retries still raises.
- T-E13: `dominant_type(triage)` applied to the merged call's own parsed `Triage` correctly repairs an item's invalid `type` to the merged response's own stated dominant type (T-E8's repair logic, unchanged, applied to the merged path).
- T-E14 (eval): faithfulness (T-E3) and decide-then-act conditioning (the merged `<ITEMS>` primarily extracts the type stated in its own `<TRIAGE>`) hold on the merged path too, and its item coverage is comparable to the old split (triage call, then extract call) path on the same fixtures — see `tests/eval/test_triage_extract_merge_eval.py`. **Not executed as part of this change** (no `ANTHROPIC_API_KEY` in the authoring environment) — run with real credentials before relying on "no extraction-quality regression" being confirmed.
- T-E15 (per-stage `max_tokens` ceiling + truncation visibility): `_parse_items_json` reports `(data, truncated)` — `truncated` is `True` only when the array had to be salvaged via `_recover_truncated_leading_objects`, and `False` both for a clean parse and for a complete array that only needed code-fence/prose stripping (a false positive here would be as unacceptable as a false negative). `run_triage_extract` threads this through as `TriageExtractResult.truncated`, logs a clear `logging.warning` naming the recovered item count when `True`, and logs nothing truncation-related when `False`. `pipeline.run_pipeline` copies it onto `KBEntry.extraction_truncated`, which `store.py` renders as a "Extraction truncated" line in both markdown export paths (mirroring the existing "Thin material" advisory) and `web/templates/entry.html` renders as a badge — see `test_store.py`'s extraction-truncated tests and `test_pipeline.py::test_extraction_truncated_flag_propagates_to_the_filed_entry`. The ceiling itself is sourced from `model_config.MODEL_MAX_OUTPUT_TOKENS` (Anthropic's published per-model output limits, cited in the PR that introduced this), resolved per stage via `model_config.resolve_stage_max_tokens` — see T-MCFG5.
...
- T-MCFG5 (per-stage `max_tokens` ceiling): `resolve_stage_max_tokens("extract")` resolves to `MODEL_MAX_OUTPUT_TOKENS[<resolved model>]` (Anthropic's published per-model output ceiling — `claude-sonnet-5`: 128,000; `claude-haiku-4-5`: 64,000) rather than a guessed number, and falls back to the flat `4096` default for a model string not in that table; every non-`extract` strong-tier stage (and `summary`) keeps the flat `4096` default regardless of which model is configured — a small classification/synthesis response doesn't need extraction's headroom. `DISTIL_MAX_TOKENS_<STAGE>` overrides one stage only, with the same isolation guarantee as `DISTIL_MODEL_<STAGE>` (T-MCFG2). `make_stage_client("extract")` constructs an `AnthropicClient` whose `.max_tokens` reflects the resolved ceiling.
...
- T-PL3: useful transcript with graph disabled stays within three LLM calls: the merged triage+extract call, link, note — triage and extraction used to be two separate calls (four total) before the merge.
...
- T-PL9 (Phase A, visible progress): `phase_callback` fires `(stage, "start")` before and `(stage, "finish")` after each stage, in pipeline order (`extract`, `normalize`, `link`, `note`, `file`, with `graph`/`canonicalize`/`concept_edges` where enabled — no separate `triage` stage anymore), without changing `timing_callback`'s existing behaviour.
...
- T-PL12 (Phase D): an entity mentioned in the merged triage+extract response flows end-to-end (canonicalized, synthesized, exported to `okf_root/entities/`) at the existing Stage 8 — the total LLM call count matches concepts-only-plus-entities exactly, with no extra call for a second transcript read.
- T-PL13 (Phase D): with `enable_entities=False`, the entities stage makes zero LLM calls and creates no entities, even though the merged call's items still carry an `entities` array.
- T-PL14 (narrative summary; concurrency per addendum Part 2 — supersedes an earlier "runs after note" placement): with a distinct cheap-tier `summary_client` injected, the strong client makes exactly its (now-merged) triage+extract+link+note calls — three, not four — and the cheap client makes exactly the narrative-summary call, zero of either landing on the other's client — the model-tier split proven by counting calls on two separate `FakeClient` instances, never by inspecting configuration.
...
- T-PL18 (narrative summary, addendum Part 2): `phase_callback` reports `("narrative_summary", "start")` FIRST — before `extract`'s own "start" — and `("narrative_summary", "finish")` only after `note`'s "finish" (right before `graph`/`file`) — reflecting that it starts at the front and is joined once the rest of the pipeline has already run, not that it executes between two adjacent stages.
...
- T-PL21 (addendum Part 2, independence): an extraction failure (exhausted retries on the merged call) still raises out of `run_pipeline` even when the concurrently-running summary succeeds — a summary success never masks an extraction failure. Conversely, a summary that fails on its own background thread never affects a successful extraction's outcome — `entry.narrative_summary` is `None` but `entry.knowledge_items` and everything else file normally.
