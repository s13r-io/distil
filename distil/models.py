...
class KBEntry(_Model):
    ...
    meta: EntryMeta
    # True when any chunk's extraction response had to be salvaged from an incomplete array (the
    # output-token cap was hit, or the connection dropped mid-stream) rather than parsed as a
    # complete response — see extract.ChunkedExtractionResult.truncated. knowledge_items still
    # holds whatever was recovered; this flag is what tells the owner that recovery, not a clean
    # stop, is why the count looks the way it does. Default False for entries filed before this
    # field existed and for entries whose extraction genuinely completed.
